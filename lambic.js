...
const Player = function(param) {
  ...
  self.die = function(report) {
    ...
    if(self.type === 'player'){
      // Enter ghost mode
      self.ghost = true;
      self.ghostTimer = 900; // 900 frames = 15 seconds at 60fps
      ...
      if(socket){
        ...
        deathMsg += '<br><i>Auto-respawn in 15 seconds, or type /respawn to respawn at home</i>';
  ...
  self.updateSpd = function() {
    ...
    // Ghosts have fixed speed but still get innaWoods for rendering
    if (self.ghost) {
      self.baseSpd = 4;
      self.maxSpd = 4;
      self.onMtn = false;
      ...
      return; // Skip all speed modifier logic
    }
    ...
    // Ghosts ignore terrain collision but still blocked by water
    if (self.ghost) {
      // Ghosts can't go underwater
      if (self.z === Z_LEVELS.OVERWORLD) {
