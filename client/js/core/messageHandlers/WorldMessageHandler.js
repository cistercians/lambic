/**
 * WorldMessageHandler.js
 * Handles world-related socket messages (world state, entities, map data, etc.)
 */

var WorldMessageHandler = {
  /**
   * Handle world-related message
   * @param {string} msgType - Message type
   * @param {Object} data - Message data
   */
  handle: function(msgType, data) {
    switch(msgType) {
      case 'previewData':
      ...
      case 'renderUpdate':
        ...
        break;
      default:
        console.warn('[WorldMessageHandler] Unknown message type:', msgType);
    }
  },
  ...
  handleSignInResponse: function(data) {
    // #region agent log
    fetch('http://127.0.0.1:7242/ingest/034ac346-9df5-4826-808c-9170d31a6b3f',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'WorldMessageHandler.js:185',message:'handleSignInResponse called',data:{success:data.success,hasData:!!data},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'G'})}).catch(()=>{});
    // #endregion
    ...
    if(data.success){
      ...
      // #region agent log
      fetch('http://127.0.0.1:7242/ingest/034ac346-9df5-4826-808c-9170d31a6b3f',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'WorldMessageHandler.js:247',message:'Sign-in successful, login overlay hidden',data:{},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'G'})}).catch(()=>{});
      // #endregion
    } else {
      // #region agent log
      fetch('http://127.0.0.1:7242/ingest/034ac346-9df5-4826-808c-9170d31a6b3f',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'WorldMessageHandler.js:248',message:'Sign-in failed, showing alert',data:{},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'G'})}).catch(()=>{});
      // #endregion
  ...
  handleRenderUpdate: function(data) {
    // Handle renderUpdate message from server
    // This is an informational message containing performance stats (deltaTime, fps, etc.)
    // Currently no-op, but could be used for client-side performance monitoring
    // The pack property may contain: { deltaTime, fps, stats }
    // Silently accept to prevent warning messages
  },
...
// Export for use in SocketMessageHandler
if (typeof module !== 'undefined' && module.exports) {
  module.exports = WorldMessageHandler;
}
